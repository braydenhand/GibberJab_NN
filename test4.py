...
def listen_for_deep_encoded_messages(duration=60, protocol_id=6):
    ...
    Args:
        duration: Maximum time to listen in seconds (will return earlier if complete message received)
    ...
    # Flag to indicate message completion
    message_completed = threading.Event()
                ...
                if result is not None:
                    ...
                    if is_compressed:
                        ...
                        if chunk_match:
                            ...
                            msg_id = f"chunked_msg_{total_chunks}"
                            ...
                            if len(chunk_buffer[msg_id]["chunks"]) == total_chunks:
                                ...
                                # Signal that we've completed receiving all chunks
                            ...
                            if decoded_message not in received_messages:
                                ...
                                # For non-chunked messages, we can signal completion immediately
                                message_completed.set()
                    else:
                        ...
                        if message not in received_messages:
                            ...
                            # For uncompressed messages, we can signal completion immediately
                            message_completed.set()
            except queue.Empty:
    ...
    try:
        ...
        print(f"Listening for encoded messages (max {duration} seconds)...")
        
        # Start the audio processing thread
        process_thread = threading.Thread(target=process_audio)
        process_thread.start()
        
        # Wait for either message completion or timeout
        message_completed.wait(timeout=duration)
    ...
    if message_completed.is_set():
        print(f"Message reception complete, returning early.")
    else:
        print(f"Timeout reached after {duration} seconds.")
    
    print(f'Returning messages: {received_messages}')
