...
class EnhancedDecoder(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, latent_dim, sequence_length,
        ...
        self.sequence_expander = nn.Linear(1, sequence_length)
    ...
    def forward(self, z):
        ...
        hidden = self.latent_to_hidden(z).unsqueeze(1)  # [batch, 1, hidden]
        
        # Expand to full sequence length
        hidden = self.sequence_expander(hidden.transpose(1, 2)).transpose(1, 2)  # [batch, seq, hidden]
...
class EnhancedNeuralTextCodec(nn.Module):
    ...
    def decode(self, byte_data):
        ...
        if self.quantize:
            ...
            z = torch.tensor(z_np, device=self.device).unsqueeze(0)
        ...
        for epoch in range(epochs):
            ...
            for batch in progress_bar:
                ...
                progress_bar.set_postfix({
                    ...
                    'vq': f"{vq_loss_val if isinstance(vq_loss_val, float) else vq_loss_val.item():.4f}" if vq_loss_val != 0 else "0.0000"
                })
...
def train_enhanced_neural_codec(corpus_path=None, output_path="enhanced_neural_codec", epochs=30):
    ...
    if corpus_path and os.path.exists(corpus_path):
        ...
        _, corpus = create_deep_corpus(corpus_size=5000000)  # Create larger corpus
    ...
    model = EnhancedNeuralTextCodec(
        ...
        embedding_dim=256,
        hidden_dim=512,
        latent_dim=128,  # Use 128 dimensions for good compression
        sequence_length=64,
        quantize=True,
    ...
    model.train_model(
        ...
        beta1=0.9,
        beta2=0.999,
        validate_every=1,  # Validate more frequently
        checkpoint_every=5
    )
